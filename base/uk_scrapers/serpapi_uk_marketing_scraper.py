...
def main():
    ...
    # Limit to 5 pages → ~50 listings total
    all_results = []
    ...
    print(f"✅ Filtered down to {len(filtered)} marketing listings before checking for closed ones...")
    ...
    open_listings = []
    for job in filtered[:60]:  # Safety cap to keep runtime short
        try:
    ...
    open_listings = open_listings[:50]
    print(f"✅ Found {len(open_listings)} open and valid listings.")
